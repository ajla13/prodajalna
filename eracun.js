...
// Vrni podrobnosti o stranki iz strankiega ID-ja
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  strankaIzId(expressSession.prijavaId, function(podatki){
    pesmiIzKosarice(zahteva, function(pesmi) {
      if (!pesmi || !podatki) {
        odgovor.sendStatus(500);
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    if(napaka2 == true){
      sporociloTekst = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
    }
    else{
      sporociloTekst = "Stranka je bila uspešno registrirana.";
    }
    odgovor.redirect("/prijava");
    odgovor.end();
  });
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: sporociloTekst, seznamStrank: stranke, seznamRacunov: racuni});  
      });
    });
})
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    expressSession.prijavaId = polja.seznamStrank;
    odgovor.redirect('/');
  });
})
